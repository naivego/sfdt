...
rtcapouttofile=gpath + r'\rtcap_' + symbol + '.txt'

codeid = "IF"

bgni = 3000
endi = 3990
dtmbgn = "2011/03/01 09:30:00"
dtmend = "2017/02/13 15:00:00"
avgski = 1
avgsk_cn = 30
maxbarback = 20
...
sgn_qsbpl = [0.35, 0]    # 趋势转多头后价格回调，仓位调整台阶水平%
sgn_qsbcv = [0.3,0.5]    # 趋势转多头后价格回调台阶对应的持仓水平
sgn_qsspl = [0.65,1.0]   # 趋势转空头后价格反弹，仓位调整台阶水平%
sgn_qsscv = [-0.3,-0.5]  # 趋势转空头后价格反弹台阶对应的持仓水平
...
stl_spl = [1.65, 2.1]  # 空头止损价位%
stl_scv = [-0.2, 0]  # 空头调整持仓到持仓水平
stl_bpl = [-0.65, -1.1]  # 多头止损价位%
stl_bcv = [0.2, 0]  # 多头调整持仓到持仓水平
...
def istoswchpcT(skiT):
    ...
    if skiT > LSTCONTSWCH_PCDTM:
        ...
        LSTCONTSWCH_KCDTM = datetime.datetime.strptime(ndtmstr, "%Y-%m-%d") + datetime.timedelta(days=3,
                                                                                                 hours=CONTSWCH_KCT / 100,
                                                                                                 minutes=CONTSWCH_KCT % 100)
...
def avgsk(i):  # 注意：本函数是计算第i根sK之前的cn根柱子的平均柱子涨幅，要求i>0
    ...
    if bi == 0:
        ...
        sumsk = abs(sk_close[bi] - sk_close[bi - 1])
...
def qspouttozbcsv(csvfpth):
    ...
    for idx in range(0, len(sk_qspds)):
        ...
        csv_writer.writerow(
            (idxt[0], idxt[1], qsprti, nscklidx, nscklitp, qspbi, qspei, qspbsdp, qspesdp, qspbp, qspep))
    csvfobw.close()
...
with open(rtcapouttofile, "w") as f:
    for idx in range(0, sk_close.size):
        outcont=[str(idx),str(sk_close.index[idx]),str(round(sk_rtcap[idx], 2)), str(round(sk_rtpctv[idx], 2)),str(round(sk_rtslots[idx], 0))]
        f.write(",".join(outcont)+'\n')
